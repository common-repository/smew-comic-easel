...
Description: s2member intigration for Comic Easel for drip feeding of comic posts. Currently hardcoded to allow s2Member restricted Comic Easel custom post type 'comic' to drip content to everyone after a set time. Only functions on Single Post. This is a child plugin for both <a href="https://wordpress.org/plugins/comic-easel/">Comic Easel</a> and <a href="https://wordpress.org/plugins/s2member/">s2Member</a>. You must have both for this to be of any use.
Version: 0.1.8
License: GPL
...
function comic_size($output)
	...
	if (get_option('image_size_medium')=='yes')
		...
		if (current_user_can($s2_access_level) || !get_option('restrict_image_size_medium'))
			{
			$size = 'medium';
			}
		}
	...
	if (get_option('image_size_large')=='yes')
		...
		if (current_user_can($s2_access_level) || !get_option('restrict_image_size_large'))
			{
			$size = 'large';
			}
		}
	...
	if (get_option('image_size_full')=='yes')
		...
		if (current_user_can($s2_access_level) || !get_option('restrict_image_size_full'))
			{
			$size = 'full';
			}
		}
